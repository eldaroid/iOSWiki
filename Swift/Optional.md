...
> Опции Swift фактически реализуются с использованием перечислений (enums) с связанными значениями ([associated values](https://github.com/eldaroid/iOSWiki/blob/master/Swift/Glossary.md#:~:text=Associated%20values)). Есть два случая: `none` и `some`, у `some` есть значение, указанное внутри необязательного.
