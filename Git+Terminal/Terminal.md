...
Чтобы поставить работающую команду на паузу нажмите комбинацию `Ctrl+Z` или `Ctrl+C`, а чтобы продолжить — `%`(пример: запустите команду `tail | head`). Чтобы закончить ввод команды, нужно нажать `Ctrl+D` (пример: запустите команду `cat`)
